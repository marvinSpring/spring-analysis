...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			...
			//将生成好的BeanDefinition对象包装成BeanDefinitionHolder对象
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		...
		//调用handler的parse方法
		//parse方法返回的是也是BeanDefinition对象，其实就是spring的埋点，可以让第三方jar包自己实现自定义的标签解析，例如dubbo
		//注意，在parse中解析完beanDefinition，是需要第三方jar手动将BeanDefinition注册到BeanDefinitionMap和beanNames中的
	...
	public BeanDefinitionHolder decorateIfRequired(
		...
		//判断属性节点是否存在对应的命名空间url且此属性节点非默认属性节点
		//example：  <bean id ="xxx" 自定义属性 = “yyyy” class="xxxxx">
		if (namespaceUri != null && !isDefaultNamespace(namespaceUri)) {
			//获取namespaceUri对应的处理实例
			//核心思想：Spring通过SPI机制,默认加载所有jar包中/META-INF/spring.handlers文件，在实现NamespaceHandler接口的类中找到匹配实例
			NamespaceHandler handler = this.readerContext.getNamespaceHandlerResolver().resolve(namespaceUri);
			if (handler != null) {
