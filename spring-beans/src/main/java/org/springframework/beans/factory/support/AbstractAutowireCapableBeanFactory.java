...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	/** 是否自动尝试解析bean之间的循环引用 *///是否spring去自动允许循环依赖
	private boolean allowCircularReferences = true;

	/**
	 * 是否在循环引用的情况下注入一个原始bean实例，
	 * 即使注入的bean最终被包装。
	 */
	private boolean allowRawInjectionDespiteWrapping = false;

	/**
	 * 要在依赖项检查和自动装配时忽略的依赖项类型，
	 * 如类对象的集合:例如，字符串。默认值为none。
	 */
	private final Set<Class<?>> ignoredDependencyTypes = new HashSet<>();

	/**
	 * 在依赖项检查和自动装配时忽略的依赖项接口，
	 * 如类对象集。缺省情况下，只有BeanFactory接口被忽略。
	 */
	private final Set<Class<?>> ignoredDependencyInterfaces = new HashSet<>();

	/**
	 * 当前创建的bean的名称，
	 * 用于从用户指定的Supplier回调触发的getBean等调用上的隐式依赖注册。
	 */
	private final NamedThreadLocal<String> currentlyCreatedBean = new NamedThreadLocal<>("Currently created bean");

	/**  未完成的FactoryBean实例的缓存:key:FactoryBean名称,val:BeanWrapper. */
	private final ConcurrentMap<String, BeanWrapper> factoryBeanInstanceCache = new ConcurrentHashMap<>();

	/** 每个工厂类的候选工厂方法缓存.,key:工厂类,val:候选工厂方法 */
	private final ConcurrentMap<Class<?>, Method[]> factoryMethodCandidateCache = new ConcurrentHashMap<>();

	/** 过滤后的PropertyDescriptors缓存:key:bean的类,value:PropertyDescriptor数组. */
	private final ConcurrentMap<Class<?>, PropertyDescriptor[]> filteredPropertyDescriptorsCache =
			new ConcurrentHashMap<>();


	/**
	 * 构造一个新的有能力去自动装配的bean工厂 AbstractAutowireCapableBeanFactory。
	 */
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			// 从BeanPostProcessors容器中找到实现InstantiationAwareBeanPostProcessor接口的类，
			// 调用其中的实例前置方法和后置方法，让BeanPostProcessor先拦截返回代理对象；
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				...
				//MergedBeanDefinitionPostProcessor 后置处理器是在bean实例化之后调用的
				try {
	...
	@Nullable
	/*
	获取所有BeanPostProcessor，并且找到实现了InstantiationAwareBeanPostProcessor的BeanPostProcessor，进行方法回调
	*/
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		/* 1）、拿到InstantiationAwareBeanPostProcessor后置处理器；
		   执行后置处理器的postProcessAfterInstantiation()*/
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
		...
		if (pvs != null) {
			//=====赋值之前：===
			//3）、应用Bean属性的值；利用setter方法为属性进行赋值；
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			//判断是否实现了 BeanNameAware\BeanClassLoaderAware\BeanFactoryAware 这些接口的bean，如果是则执行相应的方法
			invokeAwareMethods(beanName, bean);
		}
	 ...
	 * 2）、是否自定义了初始化方法；通过注解的方式添加了initMethod方法的，
	 * 例如：
